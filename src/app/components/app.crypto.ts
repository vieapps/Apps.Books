...
export class AppCrypto {
	...
	static hmacSign(text: string, key: string): string {
	...
	static urlSign(text: string, key: string): string {
	...
	static jwtEncode(jwt: any, key: string): string {
	...
	static jwtDecode(jwt: string, key: string): string {
	...
	static rsaEncrypt(text: string): string {
		return this._rsa.encrypt(text) as string;
	}

	/** Encrypts the string by AES */
	static aesEncrypt(text: string, key?: any, iv?: any): string {
	...
	static aesDecrypt(text: string, key?: any, iv?: any): string {
	...
	static initAES(key: string, iv: string) {
		this._aes.key = CryptoJS.enc.Hex.parse(key);
		this._aes.iv = CryptoJS.enc.Hex.parse(iv);
	}

	static initRSA(exponent: string, modulus: string) {
